...
V1.3 引入**确定性的 Tier-1 运行时策略引擎**，坐在宿主 agent runtime 和
每一次 tool call 之间。和 V1.x 的静态扫描器不同：扫描器跑在事前用来发
现配置漂移，运行时 guard 则是每次调用都运行，可以在灾难性操作真正执行
前**直接 Block**。

```text
OpenClaw before_tool_call
  -> clawguard-runtime 插件（TS 薄壳，每个 session 启动一次）
  -> clawguard runtime broker（子进程，stdio 上 NDJSON）
  -> PolicyEngine
     ├─ destructive_action_check        （rm -rf / dd / mkfs 等直接 Block）
     ├─ lethal_trifecta_precondition    （读/写敏感路径给 Warn）
     ├─ path_boundary_check             （forbidden_writes 直接 Block）
     ├─ rate_limit_check                （destructive 类暴增给 Warn）
     └─ prompt_injection_shape_check    （tool 结果里嗅到注入形状给 Warn/Suspect）
  -> AdapterResponse { decision, block, block_reason, evidence }
```

合并规则：`Allow < Warn < Suspect < Block`，worst-verdict-wins。规则 panic
时自动 fail-open 并写一条 `clawguard-adapter-panic` 到 stderr，确保一条
bug 规则永远不会把宿主 runtime 卡死。

### 默认策略清单

运行时引擎会加载 `~/.clawguard/policy.toml`，没有这个文件就用内置默认值：

- `[[destructive_actions.patterns]]`：基于 token 的空白边界匹配器。默认
  包含 `rm -rf`、`dd of=/dev/`、`mkfs`、`DROP TABLE`、`TRUNCATE TABLE`、
  `git push --force`、fork bomb、reverse shell、`curl | sh`、`wget -O- | sh`。
  设置 `match_in_shell_tunnel = true` 的规则会穿透最多 3 层 `sh -c` /
  `bash -c` 包装，并且对 `r''m -r''f ~` 这种 inert-quote 拆分 payload
  也有效。
- `[lethal_trifecta]`：读/写这些路径会触发 precondition。默认包含
  `~/.ssh`、`~/.aws`、`~/.config/gcloud`、`~/.openclaw/.env`、`/etc/shadow`、
  `/etc/sudoers` 等。路径匹配前会先做 dot-segment 折叠（`/etc/./shadow`
  也会命中）。
- `[path_boundary]`：`forbidden_writes` 列表（默认包含
  `~/.openclaw/hooks`、`~/.openclaw/plugins`、`~/.clawguard` 和两个审批
  文件），以及可选的 `workspace_root` 用于写越界检测。dot-segment
  折叠同样适用。
- `[rate_limit]`：对 destructive 类调用的滑动窗口计数，**按 session_id
  隔离**（引擎内部维护 `HashMap<session_id, RateLimiter>`，软上限 1024
  个 session）。默认 60 秒内 5 次。
- `[prompt_injection]`：对 canonical 化后的 tool 结果跑加权子串扫描。
  默认阈值 3，配有 role-override 标记和 instruction-override 短语的
  curated 列表。结果会先走 NFKC + 零宽字符剥离 + 空白合并的管线再打分。

### 命令

- `clawguard policy init [--force]` — 把默认清单写到 `~/.clawguard/policy.toml`（权限 0600）。
- `clawguard policy validate [--path <path>]` — 解析一个清单但不应用它，
  输出 pattern 数和敏感路径数。
- `clawguard runtime broker [--manifest <path>]` — OpenClaw 插件启动的子进程，
  按行读 JSON 事件、按行写 verdict。

### 运行时 finding kinds

Verdict 通过标准的 finding 通道下发，因此自动计入 posture score 和持久化：

| Finding kind                            | Severity | OWASP ASI | 触发条件                                           |
| --------------------------------------- | -------- | --------- | -------------------------------------------------- |
| `runtime-destructive-action`            | Critical | ASI02     | 命中 destructive pattern                           |
| `runtime-lethal-trifecta-precondition`  | High     | ASI06     | Tool call 触达 `sensitive_paths` 中的路径          |
| `runtime-path-escape`                   | High     | ASI02     | 命中 `forbidden_writes` 或越出 workspace root      |
| `runtime-rate-limit-exceeded`           | Medium   | ASI02     | 同一 session 内 destructive 类调用超过窗口阈值     |
| `runtime-prompt-injection-shape`        | Medium   | ASI07     | Tool 结果在 curated 列表上打分达到阈值             |

### OpenClaw 插件安装（预览）

运行时 guard 以独立插件形态提供（`clawguard/openclaw-plugin-runtime/`），
和已有的 SSE 告警插件 `openclaw-plugin/` 同级、互不干扰。把整个目录
拷贝到 `~/.openclaw/extensions/clawguard-runtime/` 并确保 `clawguard`
在 `PATH` 里即可。`plugin install openclaw` CLI 助手将在 Sprint 2 §6
提供，当前阶段以手工拷贝为准。
