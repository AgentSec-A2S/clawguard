...
**当前版本: v1.2.0-beta.5**（V1.3 Sprint 1 MCP 供应链加固于 2026-04-21 发布；V1.3 Sprint 2 运行时 guard 预览于 2026-04-23 进入主干）
...
- **V1.3 Sprint 2（2026-04-23 发布）** — 运行时策略 guard 在每次 tool call 发生瞬间拦截（通过 OpenClaw `before_tool_call` / `after_tool_call` hook），5 种 kind 全部映射 OWASP ASI，verdict 走标准 finding 通道因此自动计入 posture score 和 SQLite 持久化：
  - `runtime-destructive-action`（Critical, ASI02）— tokenized destructive pattern（`rm -rf`、`dd of=/dev/`、`mkfs`、`DROP TABLE`、`git push --force`、fork bomb、reverse shell、`curl | sh`、`wget -O- | sh`）在 NFKC + 零宽剥离 + 空白合并的 canonical 化后匹配到 tool-call 参数；还能穿透最多 3 层 `sh -c` / `bash -c` 包装，并击穿 `r''m -r''f ~` 这种 inert-quote 拆分
  - `runtime-lethal-trifecta-precondition`（High, ASI06）— tool call 读/写 `sensitive_paths` 中的路径（`~/.ssh`、`~/.aws`、`~/.config/gcloud`、`~/.openclaw/.env`、`/etc/shadow`、`/etc/sudoers` 等）；匹配前做 dot-segment 折叠（`/etc/./shadow` 同样命中）。发 Warn，与后续 exfil 的组合由 Sprint 3 升级
  - `runtime-path-escape`（High, ASI02）— 调用目标落在 `forbidden_writes`（默认 `~/.openclaw/hooks`、`~/.openclaw/plugins`、`~/.clawguard`、两个审批文件）或越出 `workspace_root`；dot-segment 折叠同样适用
  - `runtime-rate-limit-exceeded`（Medium, ASI02）— destructive 类调用在同一 session 内超过滑动窗口；引擎维护 per-`session_id` 的 `HashMap<session_id, RateLimiter>`，软上限 1024，攻击者轮换 session id 绕窗口会失败。默认窗口：60 秒 / 5 次
  - `runtime-prompt-injection-shape`（Medium, ASI07）— canonical 化的 tool 结果在 curated role-override / instruction-override 子串上加权打分达到阈值；canonical 化管线为 NFKC + 零宽剥离 + 空白合并。默认阈值 3
...
- 确定性 Tier-1 运行时 guard（V1.3 预览）— 坐在宿主 agent runtime 和每一次 tool call 之间：
  - 每次 `before_tool_call` / `after_tool_call` hook 都触发，而不是事前跑；灾难性动作在真正执行前**直接 Block**，不是纯告警
  - worst-verdict-wins 合并 5 条规则：destructive-action、lethal-trifecta precondition、path-boundary、per-session rate limit、prompt-injection shape
  - 规则 panic 时自动 fail-open，在 stderr 写一条 `clawguard-adapter-panic` marker；一条 bug 规则不会把 OpenClaw 卡死
  - 策略加载自 `~/.clawguard/policy.toml`（无文件时用内置默认值）；`clawguard policy validate` 在不应用的前提下解析清单
  - 以独立插件 `openclaw-plugin-runtime/` 提供（与既有 SSE 告警插件 `openclaw-plugin/` 同级、互不干扰），在每个 session 启动一个 `clawguard runtime broker` 子进程，stdio 上跑 NDJSON
  - 一键安装：`clawguard plugin install openclaw`（插件文件嵌入二进制，默认拒绝覆盖漂移文件与 symlink 目标）；`clawguard plugin status openclaw` 验证安装；同一份覆盖信息也以 `runtime_coverage` 对象出现在 `clawguard posture --json` 输出里
  - Operator runbook：[`docs/runbooks/clawguard-runtime-guard.md`](../docs/runbooks/clawguard-runtime-guard.md)
...
- `clawguard policy init [--force]`
  - 将默认运行时策略清单写入 `~/.clawguard/policy.toml`（权限 0600）
  - 主要小节：`[[destructive_actions.patterns]]`（token 空白边界匹配器；设置 `match_in_shell_tunnel = true` 的规则可穿透最多 3 层 `sh -c` / `bash -c`）、`[lethal_trifecta]`（敏感路径）、`[path_boundary]`（`forbidden_writes` + 可选 `workspace_root`）、`[rate_limit]`（滑动窗口，按 session_id 隔离）、`[prompt_injection]`（加权子串，默认阈值 3）
- `clawguard policy validate [--path <path>]`
  - 解析一个清单但不应用，输出 pattern 数和敏感路径数；解析失败时非零退出
- `clawguard runtime broker [--manifest <path>]`
  - OpenClaw 插件启动的 per-session 子进程，按行读 JSON 事件、按行写 verdict
  - 一般无需手工运行 — 请用 `clawguard plugin install openclaw` 把插件装好
- `clawguard plugin install openclaw [--force] [--dir <path>]`
  - 将运行时 guard 插件装入 `~/.openclaw/extensions/clawguard-runtime/`（`--dir` 可指定非标准目录）；插件文件已嵌入二进制，有单文件即可安装
  - 存在内容不一致的既有文件时拒绝覆盖，除非加 `--force`；始终拒绝对 symlink 目标写入，防止在扩展目录里植入的软链接陷阱
  - `clawguard --json plugin install openclaw` 输出按文件列出的结构化报告（`action: created / overwritten / unchanged`）
- `clawguard plugin status openclaw`
  - 检查三个插件文件是否全部存在、且与嵌入版本逐字节一致；`clawguard` 是否在 `PATH` 上供插件 spawn；`~/.clawguard/policy.toml` 是否存在
  - `clawguard --json plugin status openclaw` 输出完整的 status 对象
  - 同一组覆盖信息现在也出现在 `clawguard posture` 输出里（`--json` 模式下作为 `runtime_coverage` 对象），让 posture 同时反映静态扫描 findings 与运行时 guard 是否真的挂上
